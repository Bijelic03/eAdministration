...
export async function fetchEmployerByEmailAPI() {
	const response = await axiosInstance.get(`${EMPLOYERS_API_PATH}/by-email`);
	return response.data;
}
...
export async function quitJobAPI() {
	const response = await axiosInstance.put(`${EMPLOYERS_API_PATH}/quit/job`);
	return response.data;
}
