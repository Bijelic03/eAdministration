...
import { isProfessor } from "@/services/role.service";

const KurseviPage = () => {
  const { values, fetchExamRegistrationsById, gradeStudent } =
  ...
  const onEdit = async (data: any) => {
    try {
      ...
      handleApiSuccess("Student ocjenjen uspjesno!");
    } catch (error) {
          ...
          {values?.examregistrations?.length > 0 ? (
            ...
            values?.examregistrations?.map((examregistration: any) => (
              <TableRow key={examregistration.id}>
                ...
                <TableCell>
                  {examregistration?.grade || "NIJE OCJENJEN"}
                </TableCell>
            ...
            <TableRow>
              <TableCell colSpan={6}>
                Nema ispitnih prijava za ovaj ispit
              </TableCell>
            </TableRow>
          )}
        </tbody>
